...
const EntryLevel_TecnoMobile = () => {

    const [mobileBlogList, setmobileBlogList] = useState([]);
    let [mobileFullDetails, setmobileFullDetails] = useState([]);
    const [modalPopUp, setmodalPopUp] = useState(false);
    const {id} = useParams();
    let path = "/EntryLevel_TecnoMobile/";

    useEffect(() => {
        Axios.get('http://localhost:3001/EntryLevel_TecnoMobile')
            .then((response)=>{
    ...
    useEffect(() => {
        Axios.get(`http://localhost:3001/EntryLevel_TecnoMobile/${id}`)
            .then((response)=>{
    ...
    return ( 
        <section className="brand-group">
            <Link to="../EntryLevel">
                <h1><img src="/SVG/tecno.svg" alt="tecno"/> Phones</h1>
            </Link> 
...
export default EntryLevel_TecnoMobile;